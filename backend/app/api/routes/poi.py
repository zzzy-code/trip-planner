...
@router.get(
    "/photo",
    summary="获取景点图片",
    description="根据景点名称从图片服务获取图片，支持 Unsplash 和 Pexels 两个来源"
)
async def get_attraction_photo(
    name: str,
    city: str = Query("", description="城市名称"),
    provider: str = Query("auto", description="图片来源 (auto/unsplash/pexels)")
):
    """
    获取景点图片
    ...
    try:
        ...
        photo_url = image_service.get_photo_url(query, provider=provider)

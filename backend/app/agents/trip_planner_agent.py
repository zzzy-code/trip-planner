...
class MultiAgentTripPlanner:
    ...
    def plan_trip(self, request: TripRequest, callback=None) -> TripPlan:
        """
        使用多智能体协作生成旅行计划

        Args:
            request: 旅行请求
            callback: 进度回调函数 (event_name, payload)
        ...
        try:
            ...
            notify("agent_step", {
                ...
                "total_steps": 4,
                "status": "running",
                "message": f"正在使用高德地图搜索{request.city}景点..."
            })
            attraction_query = self._build_attraction_query(request)
            attraction_response = self.attraction_agent.run(attraction_query)
            print(f"景点搜索结果: {attraction_response[:200]}...\n")
            notify("attraction_found", {"city": request.city, "message": f"已成功搜集{request.city}景点信息"})
            ...
            notify("agent_step", {
                ...
                "total_steps": 4,
                "status": "running",
                "message": f"正在查询{request.city}天气预报..."
            })
            weather_query = f"请查询{request.city}的天气信息"
            weather_response = self.weather_agent.run(weather_query)
            print(f"天气查询结果: {weather_response[:200]}...\n")
            notify("weather_fetched", {"city": request.city, "message": f"已成功获取{request.city}天气预报"})
            ...
            notify("agent_step", {
                ...
                "total_steps": 4,
                "status": "running",
                "message": f"正在遴选适合的{request.accommodation}..."
            })
            hotel_query = f"请搜索{request.city}的{request.accommodation}酒店"
            hotel_response = self.hotel_agent.run(hotel_query)
            print(f"酒店搜索结果: {hotel_response[:200]}...\n")
            notify("hotel_found", {"city": request.city, "message": f"已获取{request.city}住宿推荐"})
            ...
            notify("agent_step", {
                ...
                "total_steps": 4,
                "status": "running",
                "message": "行程规划专家正在编排路线与预算..."
            })
            notify("plan_generating", {"message": "正在根据多智能体成果智能规划全面行程..."})
            ...
            self._enrich_attraction_images(trip_plan, request.city)
